...
# ─────────────────────────────────────────────────────────────
# IMPORTS OPCIONALES (no crashea si no están instalados)
# ─────────────────────────────────────────────────────────────
try:
...
.tag-green { display:inline-block; background:rgba(0,230,118,0.15); color:var(--green); border:1px solid rgba(0,230,118,0.3); border-radius:20px; padding:0.2rem 0.7rem; font-size:0.7rem; font-weight:700; margin-top:0.4rem; }
.tag-red   { display:inline-block; background:rgba(255,23,68,0.12);  color:var(--red);   border:1px solid rgba(255,23,68,0.3);  border-radius:20px; padding:0.2rem 0.7rem; font-size:0.7rem; font-weight:700; margin-top:0.4rem; }
.tag-amber { display:inline-block; background:rgba(255,171,0,0.12);  color:var(--amber); border:1px solid rgba(255,171,0,0.3);  border-radius:20px; padding:0.2rem 0.7rem; font-size:0.7rem; font-weight:700; margin-top:0.4rem; }
...
div[data-testid="stButton"]>button { background:linear-gradient(135deg,var(--indigo),#4f46e5) !important; color:white !important; border:none !important; border-radius:14px !important; font-family:'Syne',sans-serif !important; font-weight:700 !important; width:100% !important; transition:all 0.2s !important; box-shadow:0 4px 14px rgba(99,102,241,0.3) !important; }
...
def init_elevator():
    return {
        "current": 1,
        "target":  None,
        "state":   "idle",   # idle | moving | arrived | emergency
...
if "ev" not in st.session_state:
    st.session_state.ev = init_elevator()
if "bridge" not in st.session_state:
    st.session_state.bridge = WokwiBridge()
if "feedback" not in st.session_state:
    st.session_state.feedback = None
if "draw_digit" not in st.session_state:
...
# ─────────────────────────────────────────────────────────────
# ACCIÓN CENTRAL: seleccionar piso desde cualquier modalidad
# ─────────────────────────────────────────────────────────────
def action_go(floor: int, source: str):
    ...
    if ev["state"] == "moving":
        ...
        steps = abs(floor - ev["current"] if ev["target"] is None else floor - (ev["current"]))
        steps = max(1, abs(floor - (ev["current"] - (1 if ev["state"]=="moving" else 0))))
...
with tab_voz:
    ...
    <div class="info-card">
        <div class="info-row">
            ...
            <div>
                ...
                <div class="info-val">Toca el botón y di el piso</div>
                <div class="info-sub">Ej: "Ir al piso cuatro" · "Seis" · "Emergencia"</div>
            </div>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    if BOKEH_OK:
        stt_btn = BokehButton(label="🎙  Iniciar escucha", width=220, height=40)
        stt_btn.js_on_event("button_click", CustomJS(code="""
            ...
            r.onresult = function(e) {
                ...
                for (var i = e.resultIndex; i < e.results.length; ++i)
                    if (e.results[i].isFinal) v += e.results[i][0].transcript;
                if (v) document.dispatchEvent(new CustomEvent('GET_TEXT', {detail: v}));
            };
            r.start();
        """))
        result = streamlit_bokeh_events(stt_btn, events="GET_TEXT", key="voz",
                                        refresh_on_update=False, override_height=60, debounce_time=0)
        if result and "GET_TEXT" in result:
            texto = result["GET_TEXT"]
            st.markdown(f'<div class="result-pill"><span>🗣️</span><div><div style="color:#64748b;font-size:0.68rem;font-weight:600">ESCUCHÉ</div>"{texto}"</div></div>', unsafe_allow_html=True)
            parsed = parse_voice(texto)
            if "emergency" in parsed:
                emergency(ev); br.send_emergency()
        ...
        st.warning("Instala `bokeh==2.4.3` y `streamlit-bokeh-events` para activar el micrófono.")
        txt = st.text_input("O escribe el comando aquí:", key="voz_txt")
        if st.button("Procesar comando", key="voz_proc"):
            parsed = parse_voice(txt)
            if "emergency" in parsed:
                emergency(ev); br.send_emergency()
                st.session_state.feedback = ("red", ev["message"])
            elif "floor" in parsed:
                action_go(parsed["floor"], "voz (texto)")
            else:
                st.session_state.feedback = ("red", parsed.get("error",""))
            st.rerun()
...
with tab_draw:
    ...
    if CANVAS_OK:
        ...
        st.warning("Instala `streamlit-drawable-canvas` para activar el tablero de dibujo.")
